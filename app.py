...
st.markdown(
    ...
    /* Компактная стилизация меню контекста */
...
# =====================================================================
# ВЫГРУЗКА НЕДВИЖИМОСТИ С ЖЕСТКОЙ ФИЛЬТРАЦИЕЙ МУСОРА В UI
# =====================================================================
def fetch_real_estate_listings(
    ...
    try:
        ...
        # ЖЕСТКИЙ СПИСОК СТОП-СЛОВ ДЛЯ МГНОВЕННОГО СКРЫТИЯ МУСОРА
        TRASH_KEYWORDS = [
            "подработка", "підробіток", "консультаци", "консультаці", "петици", "петиці",
            "вакансия", "вакансія", "работа", "робота", "клининг", "клинінг", "переезд", "переїзд",
            "заработок", "заробіток", "услуги", "послуги", "ремонт", "инстаграм", "instagram"
        ]
        ...
        for pt in points:
            ...
            # 1. Быстрый локальный фильтр по стоп-словам (скрывает старый спам)
            if any(tk in raw_text for tk in TRASH_KEYWORDS):
                continue

            # 2. Фильтрация по типу поста (offer vs demand vs trash)
